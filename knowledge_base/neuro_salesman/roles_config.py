...
Ничего не придумывай. Если Приветствия нет — верни пустую строку "".
...
Ничего не придумывай от себя, если потребностей не выявлено, то верни абсолютно пустую строку "".
...
# Ничего не придумывай от себя, если преимуществ не выявлено, то верни абсолютно пустую строку "".
# Порядок отчета: в своем ответе предоставь только список topic phrase в виде строки с разделителями (запятые) без объяснений и слов «нет», «ничего» и т.п.
            "instructions": '''
Действуй строго по правилам:

1. Проанализируй Предыдущий ответ Менеджера отдела продаж.
2. Найди в нём только названные преимущества (если они есть).
3. Если преимущества есть — верни их в виде строки, где преимущества перечислены через запятую.
   Пример: "удобное расписание, диплом государственного образца, опытные преподаватели"
4. Если преимуществ НЕТ — верни точно пустую строку "" (две кавычки подряд без пробелов, без слов, без пояснений).
5. Не добавляй никаких других слов, комментариев, объяснений или фраз вроде "нет преимуществ".
''',
        },
...
и напиши отработанные возражения.
...
1. Проанализируй Предыдущий ответ Менеджера отдела продаж.
2. Найди в нём только названные отработки возражений (если они есть).
3. Если отработки возражений есть — верни их в виде строки, где отработки возражений перечислены через запятую.
   Пример: "удобное расписание, диплом государственного образца, опытные преподаватели"
4. Если отработки возражений НЕТ — верни точно пустую строку "" (две кавычки подряд без пробелов, без слов, без пояснений).
5. Не добавляй никаких других слов, комментариев, объяснений или фраз вроде "нет отработки возражений".
''',
        },
        # Выявление ТАРИФА
        "tariffs": {
...
''',
            "instructions": '''
Давай действовать последовательно:
Проанализируй текст и выяви topic phrase (если они есть) и напиши их.
Ничего не придумывай от себя, если topic phrase не выявлено, то верни абсолютно пустую строку "".
Порядок отчета: в своем ответе предоставь только список topic phrase в виде строки с разделителями (запятые) без объяснений.
''',
        },
