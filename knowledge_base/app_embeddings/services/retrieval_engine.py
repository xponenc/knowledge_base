...
def reformulate_question(
    ...
    reformulate_prompt = PromptTemplate(
        ...
        template="""
            ...
            Твоя задача:
            1. Определи, самодостаточен ли текущий вопрос: можно ли по нему одному понять, что именно ищет пользователь?
                - Если **да**, выдели из него ключевые **смысловые слова** (через пробел), которые отражают суть вопроса.
                - Если **нет**, используй контекст из истории, чтобы восстановить полный смысл вопроса, и затем выдели ключевые слова, которые помогут найти нужные документы.
            
            ⚠️ Возвращай **ТОЛЬКО ключевые слова**, разделённые пробелами.  
            ⚠️ Не добавляй пояснений, заголовков, пунктов.  
            ⚠️ Не пиши полные фразы или предложения — только термины.
            ...
            → Ответ: генеральный директор иванов иван иванович приказ о назначении
            ...
            → Ответ: эпидемиология профессиональная переподготовка
