...
def answer_index(system, topic, search_index, verbose=True):
    ...
    docs = search_index.similarity_search_with_score(topic, k=10)
    print("ОТВЕТ FAISS")
    ...
    # Пропускаем всё, что менее схоже (например, cosine distance > 0.4)
    filtered_docs = [(doc, score) for doc, score in docs if score < 0.8]

    # Далее как обычно
    pairs = [(topic, doc.page_content) for doc, _ in filtered_docs]
    scores = reranker.predict(pairs)
    reranked = sorted(zip(filtered_docs, scores), key=lambda x: x[1], reverse=True)
    top_docs = [doc for (doc, _), _ in reranked[:5]]
    print("ОТВЕТ RANKER")
    for document, score in reranked[:5]:
        print("\n")
        print(score)
        print(document)
    print("КОНЕЦ RANKER \n\n\n")
